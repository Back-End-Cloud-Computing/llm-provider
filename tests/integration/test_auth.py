...
async def real_auth_client(monkeypatch):
...
async def test_protected_route_without_token_returns_401(real_auth_client):
    ...
    response = await client.post("/generate", json={"prompt": "descreva um produto"})
...
async def test_protected_route_with_valid_token_succeeds(real_auth_client):
    ...
    response = await client.post(
        "/generate", json={"prompt": "descreva um produto"}, headers={"Authorization": f"Bearer {token}"}
    )
